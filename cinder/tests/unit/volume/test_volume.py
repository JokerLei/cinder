...
class VolumeTestCase(base.BaseVolumeTestCase):
    ...
    def test_rebuild_volume_again(self):
        request_spec = {
            'image': fake.IMAGE_ID,
            'status': 'error'
        }
        ...
        volume = tests_utils.create_volume(self.context, size=2,
                                           host=CONF.host, **request_spec)
        volume.previous_status = 'avaliable'
        volume_api = cinder.volume.api.API()
        volume_api.rebuild(self.context, volume, request_spec['image'])
        volume.refresh()
        self.assertEqual('rebuilding', volume.status)
        self.volume.delete_volume(self.context, volume)

    def test_rebuild_volume_driver_not_initialized(self):
        request_spec = {
            'image': fake.IMAGE_ID,
            'status': 'reserved'
        }
        ...
        self.assertRaises(exception.DriverNotInitialized,
                          self.volume.rebuild_volume,
                          self.context, volume, request_spec,
                          )
    ...
    def test_rebuild_voluem_with_snapshot_id(self):
        volume = tests_utils.create_volume(self.context, size=2,
                                           host=CONF.host)

        volume.metadata['snapshop_id'] = fake.SNAPSHOT_ID
        image_id = fake.IMAGE_ID
        volume.refresh()
        self.volume_api.rebuild(self.context,volume,image_id)
        self.assertEqual('available', volume.status)

    def test_rebuild_volume_in_use_with_exception(self):
        volume = tests_utils.create_volume(self.context, size=2,
                                           status='creating', host=CONF.host)
        self.volume.create_volume(self.context, volume)
        instance_uuid = '12345678-1234-5678-1234-567812345678'
        ...
        db.volume_attached(self.context, attachment.id, instance_uuid,
                           'fake-host', 'vda')
        volume.refresh()
        self._test_rebuild_volume_in_use_with_exception(volume)
        self.volume.detach_volume(self.context, volume.id, attachment.id)
        self.volume.delete_volume(self.context, volume)

    def _test_rebuild_volume_in_use_with_exception(self, volume):
        image_id = fake.IMAGE_ID
        self.assertRaises(exception.VolumeStatusCanNotBeRebuild,
                          self.volume_api.rebuild,
                          self.context, volume, image_id
                          )
