...
class VolumeManager(manager.CleanableManager,
    ...
    def rebuild_volume(self, context, volume, request_spec):
        ...
        self._notify_about_volume_usage(context, volume, "detach.start")
        try:
            snapshots = len(objects.SnapshotList.get_all_for_volume(context,
                                                                    volume.id))
            if snapshots:
                msg = _("Volume with snapshots can\'t be rebuild.")
                LOG.error("Unable to rebuild busy volume.",
                          resource=volume)
                raise exception.VolumeHasSnapshot(volume=volume.id, reason=msg)
        ...
        try:
            ...
            raise
        self.db.volume_glance_metadata_delete_by_volume(context, volume.id)
        self.create_volume(context, volume, request_spec)
        self.db.volume_update(context, volume.id,
                              {'status': status})
        LOG.info("Rebuild-volume rebuild successfully.", resource=volume)
