...
class API(base.Base):
    ...
    def rebuild(self, context, volume, image_id):
        context.authorize(vol_policy.CREATE_FROM_IMAGE_POLICY)
        image_meta = self.image_service.show(context, image_id)
        volume_meta = self.get(context, volume.id)
        status = volume_meta['status']
        possiblestat = ['reserved', 'available', 'error']
        if status not in possiblestat:
            s_msg = _('Volume status is %(status)s .') % {'status': status}
            raise exception.VolumeStatusCanNotBeRebuild(
                volume=volume.id, reason=s_msg)
        if status == 'error':
        ...
        status_temp = 'rebuilding'
        self.db.volume_update(context, volume.id,
                              {'status': status_temp})
        if volume_meta.snapshot_id:
            volume_meta.snapshot_id = None
            volume_meta.save()

        if volume_meta.source_volid:
            volume_meta.source_volid = None
            volume_meta.source_volstatus = None
            volume_meta.save()

        size = utils.as_int(volume.get('size'), quiet=False)
        ...
        if size < min_disk:
            ...
            raise exception.InvalidInput(reason=msg)
        self.volume_rpcapi.rebuild_volume(context, volume, request_spec)
        LOG.info("Rebuild volume request issued successfully.",
                 resource=volume)
