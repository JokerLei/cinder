...
class ISCSITarget(driver.Target):
    ...
    def create_export(self, context, volume, volume_path, initiator_ip=None):
    ...
    def extend_target(self, volume):
        ...
        if volume.volume_attachment:
            # even attachment is empty, it will still try to
            # update tgt target which is problematic
            for attachment in volume.volume_attachment.objects:
                if 'connection_info' in vars(attachment) and attachment.connection_info:
